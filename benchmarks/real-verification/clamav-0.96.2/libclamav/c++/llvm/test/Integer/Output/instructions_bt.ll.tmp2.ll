...
define i39 @test_extractelement(<4 x i39> %V) {
  %R = extractelement <4 x i39> %V, i32 1         ; <i39> [#uses=1]
  ret i39 %R
}

define <4 x i39> @test_insertelement(<4 x i39> %V) {
  %R = insertelement <4 x i39> %V, i39 0, i32 0   ; <<4 x i39>> [#uses=1]
  ret <4 x i39> %R
}

define <4 x i39> @test_shufflevector_u(<4 x i39> %V) {
  %R = shufflevector <4 x i39> %V, <4 x i39> %V, <4 x i32> <i32 1, i32 undef, i32 7, i32 2> ; <<4 x i39>> [#uses=1]
  ret <4 x i39> %R
}
