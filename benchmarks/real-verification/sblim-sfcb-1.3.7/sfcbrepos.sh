...
usage() 
{
    echo "usage: $0 [-h] [-f] [-i] [-b backendopt] [-c cimschemadir] [-s stagingdir] [-r registrationdir] " 1>&2 
}

args=`getopt fhtzib:s:r:c:X: $*`
rc=$?
...
then
    ...
    echo -e "\t-s specify staging directory [${prefix}/var/lib/sfcb/stage]"
    echo -e "\t-r specify repository directory [${prefix}/var/lib/sfcb/registration]"
    echo -e "\t-c specify directory containing CIM Schema MOFs [${prefix}/share/sfcb/CIM]"
...
if [ -z "$stagingdir" ]
then
    stagingdir=${DESTDIR}${prefix}/var/lib/sfcb/stage
fi

if [ -z "$registrationdir" ]
then
    registrationdir=${DESTDIR}${prefix}/var/lib/sfcb/registration
fi

if [ -z "$cimschemadir" ]
then
    cimschemadir=${DESTDIR}${prefix}/share/sfcb/CIM
fi
	...
	then
        ...
        then
            ...
            then
                ...
                do
                    sfcbinst2mof -n $namespace -c $instfile -o $instmigfile -r $registrationdir/repository.previous/ -g ${DESTDIR}${prefix}/etc/sfcb/sfcb.cfg 2> /dev/null
                done
            fi
        fi
        if [ -e $instmigfile ]
	    ...
	    if ! sfcbmof -d $repositorydir -n $namespace -o classSchemas -I $cimschemadir -i CIM_Schema.mof $reduced $backendopt $instmigopt $mofdir/*.mof $globalmofs
	    then
            rm -f $instmigfile 2> /dev/null
